...
class PitReading(BaseModel):
    ...
    def excess_or_shortage(self):
        actual_stock = self.actual_closing_stock
        if actual_stock is not None:
            actual_closing_stock_decimal = Decimal(str(actual_stock))
    ...
    def save(self, *args, **kwargs):
        ...
        if actual_stock and actual_stock > self.opening_stock:
            self.supply = actual_stock - self.opening_stock
            self.closing_stock = actual_stock
        else:
            self.supply = 0
    ...
    def calculate_closing_stock(self):
        ...
        liters_sold = sum(
            pump_reading.liters_sold for pump in pumps for pump_reading in pump.readings.all())
