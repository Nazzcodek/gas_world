...
class PitSerializer(serializers.ModelSerializer):
    pit_product = serializers.PrimaryKeyRelatedField(queryset=Product.objects.all())
    product_name = serializers.SerializerMethodField()

    class Meta:
        model = Pit
        fields = ['id', 'name', 'pit_product', 'station', 'current_volume', 'max_volume', 'product_name', 'created_at', 'updated_at']
...
class PitReadingSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        read_only_fields = ['reading_pit', 'excess_or_shortage', 'opening_stock', 'timestamp', 'closing_stock', 'supply']
    ...
    def update(self, instance, validated_data):
        instance.actual_closing_stock = validated_data.get('actual_closing_stock', instance.actual_closing_stock)
        instance.save()
        return instance
